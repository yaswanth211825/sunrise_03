      ...
      <div className="container-site">
        <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-8 mb-14">
          <SectionHeader
            ...
            title="What Most Contractors<br /><em>Don&rsquo;t Tell You</em>"
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {educationCards.map((card, i) => {
            ...
            const colorClass = categoryColors[card.category] ?? "text-brand-gold bg-brand-gold/10 border-brand-gold/20";
            ...
            return (
              <motion.article
                ...
                className={`glass-card rounded-[1.2rem] p-5 cursor-pointer gold-border-hover transition-all duration-300 ${
                  isOpen ? "lg:col-span-2 row-span-1" : ""
                }`}
                onClick={() => setExpanded(isOpen ? null : card.id)}
                role="button"
                aria-expanded={isOpen}
                tabIndex={0}
                onKeyDown={(e) => e.key === "Enter" && setExpanded(isOpen ? null : card.id)}
              >
                {/* Header */}
                <div className="flex items-start justify-between mb-4">
                  ...
                  <span className={`font-body text-[10px] tracking-[0.15em] uppercase border rounded-full px-2 py-0.5 ${colorClass}`}>
                    {card.category}
                  </span>
                </div>
                ...
                {/* Expand hint */}
                <p className="font-body text-[10px] text-brand-muted/60 mt-3">
