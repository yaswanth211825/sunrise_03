...
const gradeConfig = {
  Best: { color: "text-emerald-400", bar: "bg-emerald-400", label: "Recommended" },
  Good: { color: "text-amber-400", bar: "bg-amber-400", label: "Suitable" },
  Caution: { color: "text-red-400", bar: "bg-red-400", label: "Use with care" },
};
...
function DurabilityBar({ value }: { value: number }) {
  return (
    <div className="flex items-center gap-2">
      ...
      <span className="font-body text-xs text-brand-muted w-8 text-right">{value}%</span>
    </div>
  );
}
...
function MaterialCard({ item }: { item: MaterialItem }) {
  ...
  return (
    <motion.div
      ...
      className="glass-card-light rounded-xl p-4 border border-brand-border gold-border-hover"
    >
      ...
      <div className="space-y-1.5 mt-3">
        ...
        {item.avoid && (
          <div>
            <span className="font-body text-[10px] text-red-400/80 uppercase tracking-widest">
              Avoid
            </span>
      ...
      <div className="container-site">
        <div className="mb-12 flex flex-col lg:flex-row lg:items-end lg:justify-between gap-6">
          <SectionHeader
            ...
            title="Know What Goes<br /><em>Into Your Home</em>"
          ...
          {materialCategories.map((cat) => {
            ...
            return (
              <button
                ...
                className={`shrink-0 flex items-center gap-2 px-5 py-2.5 rounded-full font-body text-sm font-medium transition-all duration-300 ${
                  isActive
                    ? "bg-brand-gold text-brand-dark"
                    : "bg-brand-surface text-brand-muted hover:text-brand-cream border border-brand-border"
                }`}
              >
                {Icon && <Icon size={14} />}
                {cat.title}
        ...
        >
          ...
          <a
            href="#cta"
            className="shrink-0 inline-flex items-center gap-2 bg-brand-gold text-brand-dark rounded-full px-6 py-3 text-sm font-body font-medium hover:bg-brand-gold-light transition-colors"
          >
            Get a Consultation
          </a>
        </motion.div>
